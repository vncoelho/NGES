...
\begin{figure}[http]
	\begin{center}
		\begin{tikzpicture}[scale=.056]
			...
			\path[->, dashed]		  (n10) edge (n8);
			\path[->,teal]		  (n8)  edge (n19)
					  (n19) edge (n9)
					  (n9)  edge (n2)
					  (n2)  edge (n0);
			...
			\path[->,teal] (m0)  edge (m1);
			\path[->,teal,very thick]		  (m1) edge (m8);
			\path[->,teal]		  (m8)  edge (m19)
					  (m19) edge (m9)
					  (m9)  edge (m2)
					  (m2)  edge (m0);

			\path[->] (m0)  edge (m10);
			\path[->,very thick]		  (m10)  edge (m6);
			\path[->]		  (m6)  edge (m16)
					  (m16) edge (m11)
					  (m11) edge (m5)
					  (m5)  edge (m18)
			...
			\node[rectangle, scale=.8, teal] at (102,29) {Route 3};
		...
		\end{tikzpicture}
		\caption{Example of \textit{Swap}(1,1) Move}\label{fig:EVSwap}
	\end{center}
\end{figure}
